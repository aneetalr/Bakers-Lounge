...
public partial class Baker_Default : System.Web.UI.Page
{
    ...
    protected void grd_deliveryboy_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string ins = "insert into tbl_assignborder(assignborder_date,assignborder_status,cakebook_id,deliveryboy_id) values('" + DateTime.Now.ToShortDateString() + "',0,'" + Session["cbid"] + "','" + id + "')";
            obj.ExecuteCommands(ins);
            string update = "update tbl_cakebook set cakebook_dstatus=1 where cakebook_id='"+Session["cbid"]+"'";
            obj.ExecuteCommands(update);
            Response.Write("<script>alert('Assigned')</script>");
            Response.Redirect("PickupCake.aspx");
        }
    }
