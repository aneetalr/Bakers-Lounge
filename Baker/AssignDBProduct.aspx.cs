...
public partial class Baker_Default : System.Web.UI.Page
{
    ...
    protected void grd_deliveryboy_SelectedIndexChanged(object sender, EventArgs e)
    {
    ...
    protected void grd_deliveryboy_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            string ins = "insert into tbl_assignborder(assignborder_date,assignborder_status,productbook_id,deliveryboy_id) values('" + DateTime.Now.ToShortDateString() + "',0,'" + Session["pbid"] + "','" + id + "')";
            obj.ExecuteCommands(ins);
            string update = "update tbl_productbook set productbook_dstatus=1 where productbook_id='" + Session["pbid"] + "'";
            obj.ExecuteCommands(update);
            Response.Write("<script>alert('Assigned')</script>");
            Response.Redirect("PickupProduct.aspx");
        }
    }
}